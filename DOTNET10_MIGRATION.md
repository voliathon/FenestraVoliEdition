# 🏗️ Architecture & .NET 10 Migration History

Fenestra (Voli Edition) represents a massive architectural shift from legacy frameworks to modern, high-performance toolchains. This document outlines the critical changes made to modernize the codebase.
...
Upgrading to the Long-Term Support (LTS) release of .NET 10 required several critical architectural shifts, primarily concerning security and inter-process communication between the UI and the elevated background process (`windower.exe`).
